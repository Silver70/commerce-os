...
import { ChevronRightIcon, PlusIcon, TicketIcon } from "lucide-react";
...
import { computeDiscountStatus, formatScope, formatValue } from "../utils";
...
export function DiscountListPage() {
  const discounts: Discount[] = useSuspenseQuery(discountsQueryOptions()).data;
  const [tab, setTab] = React.useState<DiscountStatus>("active");

  const counts: Record<DiscountStatus, number> = {
    active: discounts.filter((d) => computeDiscountStatus(d) === "active")
      .length,
    scheduled: discounts.filter((d) => computeDiscountStatus(d) === "scheduled")
      .length,
    expired: discounts.filter((d) => computeDiscountStatus(d) === "expired")
      .length,
  };
