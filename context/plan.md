...
**Audit service:**
```ts
log({ entityType, entityId, action, actorType, actorId, changes, organizationId, ipAddress })
```
...
interface PaymentProvider {
  createPaymentIntent(amount: number, currency: string, metadata: Record<string, string>): Promise<{ clientSecret: string; paymentIntentId: string }>
  capturePayment(paymentIntentId: string): Promise<void>
  refundPayment(chargeId: string, amount: number, reason?: string): Promise<{ refundId: string }>
